...
# ╔═╡ 4729602a-e781-11ef-2ad2-cd600ee5bd9d
begin
...
# ╔═╡ 5f8a132b-2e2a-4610-9659-4d3e3d7edb11
begin
	FILL_NAME = "Tritium_4"

	xlsx_filepath = "/home/chernov/Downloads/Compilation_Tr4.xlsx"
	xf = XLSX.readxlsx(xlsx_filepath)
end

# ╔═╡ 02e67f23-74d3-41f0-9a99-ba7d7401d307
coeffs = let
	set = xf["Лист1"]["A"][2:end]
	a = xf["Лист1"]["B"][2:end]
	b = xf["Лист1"]["C"][2:end]
	c = xf["Лист1"]["D"][2:end]
	coeffs = DataFrame(set=set, a=a, b=b, c=c)
	filter!(row -> !ismissing(row.set), coeffs)
	coeffs
end

# ╔═╡ f46450d2-5d17-4b3b-a723-11a5c2c2ed3c
converted = let
	converted = SortedDict()
	foreach(row -> begin
		converted["set_$(Int(row.set))"] = Dict("corr_coef" => [row.a, row.b, row.c])
	end, eachrow(coeffs))
	Dict(FILL_NAME => converted)
end

# ╔═╡ ab4b8d92-b86a-4512-bcf5-e6049076acd6
write("$(splitext(xlsx_filepath)[1]).json", JSON.json(converted, 2))
...
manifest_format = "2.0"
project_hash = "3464b8b07f98e25cf86b8572defebd2b86746e92"
