...
use processing::{
    numass::{self, protos::rsb_event::Point},
    preprocess::{emulate_fir, frame_to_waveform, Preprocess},
    process::{Algorithm, TRAPEZOID_DEFAULT},
    storage::load_meta,
};
...
async fn main() {
    ...
    let pb = Arc::new(Mutex::new(indicatif::ProgressBar::new(
        std::fs::read_dir(&data_root).unwrap().count() as u64,
    )));
    let handles = std::fs::read_dir(&data_root)
        .unwrap()
        .flatten()
        .map(|file| {
            ...
            tokio::spawn(async move {
                if let Ok(DFMessage {
                    meta: numass::Reply::AcquirePoint { start_time, .. },
                    data: Some(data),
                }) = dataforge::read_df_message::<numass::Reply>(
                    &mut tokio::fs::File::open(file.path()).await.unwrap(),
                )
                .await
                {
                    // println!("{file:?}")
                    ...
                    point.channels.iter().for_each(|ch| {
                        let baseline = ch
                            ...
                            .map(|val| (val - baseline[ch.id as usize]) as f64)
                            .collect::<Vec<_>>();
